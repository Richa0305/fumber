...
class MainGameScene: SKScene,GADBannerViewDelegate {
    ...
    func addNewNumberBlock(superNode:SKShapeNode) -> SKShapeNode{
        ...
        //let mainNode = SKShapeNode(rect: CGRect(x: 0, y: 0, width: self.nodeIntSize - 10, height: self.nodeIntSize - 10), cornerRadius: 10.0)
        
        let highscore = UserDefaults.standard.integer(forKey: "HighScore")
        let randomNumberForColorCodes = arc4random()%3
        
        mainNode.fillColor = SKColor(hex: colorcodes[Int(randomNumberForColorCodes)])
        //mainNode = SKShapeNode(color: UIColor.red, size:CGSize(width: CGFloat(self.nodeIntSize - 10), height: CGFloat(self.nodeIntSize - 10)))
        ...
        var blockNumber = Int(myLabel.text!)!
        let blockColor =  mainNode.fillColor
        print("randomNumberForColorCodes \(randomNumberForColorCodes)")
        if randomNumberForColorCodes == 0 {
